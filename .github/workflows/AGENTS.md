...
| File | Description |
|------|-------------|
| `ci.yml` | `ci` workflow: colcon build (domain-tree paths), flake8 (max 120, non-gating), pytest the core-domain suites (`runtime/gateway/test`, `runtime/events/test`, `runtime/features/test`, `hmi/web_common/test`, `contracts/foundation/test`) and `products/pinky_pro/test` (D-196), `src/site/fleet/test`, `src/sim/gz_sim/test`, repo `test/`, `core` boot smoke, slam_toolbox SaveMap type guard. D-134 rehearsal workflows re-run the same procedure on other runners |
...
- `src/`, `src/runtime/gateway/test/`, `src/runtime/events/test/`, `src/runtime/features/test/`, `src/hmi/web_common/test/`, `src/site/fleet/test/`, `src/sim/gz_sim/test/`, `test/`

...
def test_games_overhead_cv2_does_not_close_the_robot_camera_adr():
    ...
    overhead = ROOT / "src" / "apps" / "games" / "games" / "host" / "overhead.py"
    assert "import cv2" in overhead.read_text(encoding="utf-8")
    homography = ROOT / "src" / "apps" / "games" / "games" / "field" / "homography.py"
...
def test_core_dockerfile_does_not_copy_games():
    ...
    assert (ROOT / "src" / "apps" / "games" / "package.xml").is_file()


def test_core_has_no_soccer_robot_mode():
    """D-90: RobotMode.SOCCER 없음."""
    schemas = (ROOT / "src" / "core" / "core_common" / "core_common" / "protocol" / "schemas.py").read_text(
        encoding="utf-8"
    )
...
def test_fleet_does_not_own_the_match_and_games_has_no_fleet_start():
    ...
    for path in fleet.rglob("*.html"):
        ...
        assert "games" not in text
    games_web = ROOT / "src" / "apps" / "games" / "games" / "web" / "index.html"
    html = games_web.read_text(encoding="utf-8").lower()
    assert "fleet" not in html
    cli = (ROOT / "src" / "apps" / "games" / "games" / "cli.py").read_text(encoding="utf-8")
    assert "fleet" not in cli
    loop = (ROOT / "src" / "apps" / "games" / "games" / "host" / "loop.py").read_text(encoding="utf-8")
...
def test_stair_presets_are_host_not_field_go():
    ...
    cli = (ROOT / "src" / "apps" / "games" / "games" / "cli.py").read_text(encoding="utf-8")
    assert '"--stair"' in cli
    progress = (ROOT / "src" / "apps" / "games" / "progress.md").read_text(encoding="utf-8")
...
def test_games_board_is_not_the_core_dashboard():
    ...
    core_web = ROOT / "src" / "core" / "core_api_web" / "core_api_web" / "web"
    games_web = ROOT / "src" / "apps" / "games" / "games" / "web"
    ...
    app = (ROOT / "src" / "core" / "core_api_web" / "core_api_web" / "api" / "app.py").read_text(encoding="utf-8")

...
from rosy_interfaces.srv import Emotion

from .info_screen import render as render_info
from .rosy_lcd import LCD

# rosy_core가 latch로 발행하므로 늦게 떠도 현재 모드를 즉시 받는다 (PWR-003).
...
class RosyEmotion(Node):
    def __init__(self):
        ...
        self.emotion_path = os.path.join(get_package_share_directory('rosy_emotion'), 'emotion')
...
def main(args=None):
    ...
    rosy_emotion_node = RosyEmotion()

    try:
        rclpy.spin(rosy_emotion_node)
    except KeyboardInterrupt:
        rosy_emotion_node.get_logger().info("KeyboardInterrupt, shutting down.")
    finally:
        rosy_emotion_node.lcd.clear()
        rosy_emotion_node.destroy_node()
        rclpy.shutdown()

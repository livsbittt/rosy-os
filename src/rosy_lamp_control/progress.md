...
gates:
  SOURCE:
    ...
    cmd: "python -m pytest src/rosy_lamp_control/test/test_lamp_package_contract.py -q"
  LOCAL:
    state: GO
...
- C++ WS2811 lamp 드라이버. GPIO 19 / DMA 10에 8개 LED, `SetLamp` 서비스와 `ColorRGBA` 구독을 제공한다. Gazebo 대응은 `rosy_gz_sim/plugins/gz_lamp_control_plugin.cpp`.
...
- `deploy/robot/Dockerfile`에는 core/io 두 이미지만 있고 rosy_lamp_control을 포함하지 않는다. `rpi_ws281x`(`ws2811.h`)는 desktop에 없어 빌드되지 않는다.

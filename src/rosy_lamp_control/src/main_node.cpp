...
#include "rosy_interfaces/srv/set_lamp.hpp"
...
class RosyLampControl : public rclcpp::Node
{
    public:
        RosyLampControl() : Node("rosy_lamp_control")
        {
            ...
            srv_set_lamp_ = this->create_service<rosy_interfaces::srv::SetLamp>("set_lamp",
                std::bind(&RosyLampControl::callback_set_lamp, this, std::placeholders::_1, std::placeholders::_2)
            );

            auto period = std::chrono::duration<double>(1.0 / 100.0);
            timer_ = this->create_wall_timer(period, std::bind(&RosyLampControl::timer_callback, this));
    ...
    private:
        void callback_set_lamp(const std::shared_ptr<rosy_interfaces::srv::SetLamp::Request> req,
                                        std::shared_ptr<rosy_interfaces::srv::SetLamp::Response> res)
        {
    ...
    private:
        ...
        rclcpp::Service<rosy_interfaces::srv::SetLamp>::SharedPtr srv_set_lamp_;
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<RosyLampControl>());

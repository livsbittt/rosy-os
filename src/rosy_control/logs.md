...
2026-09-15 이전 이력은 [흡수 실행 계획](../../docs/plans/2026-09-12-rosy-control-absorption-plan.md), [흡수 결과](../../docs/plans/2026-09-12-control-absorption-results.md)와 `git log -- src/rosy_control`를 본다.

## 2026-09-15 · uncommitted · docs(harness): start the rosy_control harness record
- 변경: `progress.md`, `logs.md` 추가
- 증거: `python3 -m pytest test/test_control_absorption_package.py -q` 6 passed; `PYTHONPATH=src/rosy_control:src python3 -m pytest src/rosy_control/test -q` 993 passed, 26 skipped, 2 failed (2026-09-15 Windows, 미커밋 WIP 포함 작업 트리)
...
- 증거: 실패 2건 단독 재실행 2 passed(저장소 루트, 패키지 디렉터리 양쪽); Windows에서 `PYTHONPATH=src/rosy_control;src`로 `python -m pytest src/rosy_control/test -q` 995 passed, 26 skipped
...
- 증거: `PYTHONPATH=src/rosy_control;src python -m pytest src/rosy_control/test -q` 998 passed, 26 skipped (2026-09-17 Windows, 미커밋 WIP 포함 작업 트리)
...
- 증거: `PYTHONPATH=src/rosy_control;src python -m pytest src/rosy_control/test/test_map_raster_color_contract.py test/test_control_launch_boundary.py -q`

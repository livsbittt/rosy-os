# rosy_control inside Rosy OS

This is the absorbed ROS 2 package under `Rosy OS/src/rosy_control`.
It supplies reusable sensing, OpenCV preprocessing, calibration, planning,
and safety evidence. `rosy_core` owns the external API, policy consumer, and
...
- `rosy_control/control`, `planning`, and `sensing` contain ROS-free decisions
  and deterministic tests.
- `rosy_control/safety` can run as a sensor-only worker owned by CORE. It must
...
- `rosy_core` binds accepted evidence to the policy and publishes the final
  command. The Device Compose profiles keep core, motor, and hardware access
  separate.
- Legacy `rosy_control` launch and console entry points remain only for parity
  tests. Never start the legacy final publisher beside `rosy_core`.

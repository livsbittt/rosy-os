...
This is the absorbed ROS 2 package in `Rosy OS/src/rosy_control`. It owns
reusable sensing, OpenCV preprocessing, calibration records, planning, and
safety evidence. `rosy_core` owns the external API, the policy consumer, and
...
- Legacy launch and console entry points remain for parity tests only. Never
  start their final publisher beside `rosy_core`.

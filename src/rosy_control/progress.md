...
gates:
  ...
  LOCAL:
    ...
    cmd: "PYTHONPATH=src/rosy_control:src python3 -m pytest src/rosy_control/test -q"
  ROS-SIM:
    state: HOLD
  ...
  FIELD:
    state: PARKED
adrs: [D-37, D-38, D-40, D-42, D-47, D-50, D-57, D-58, D-77]
plans:
...
- 흡수된 Control sensor adapter는 기본 비활성이며, 켜면 Device 보정 generation에 묶인다(D-47; `rosy_core/progress.md` 참조).
...
1. ROS Jazzy container에서 rosy_control 노드 그래프 시험을 재실행해 ROS-SIM을 되돌린다.
2. 서명된 ARM64 artifact 발행 후 Pi readback으로 Control sensor adapter 활성화 경로를 확인한다(ARTIFACT → DEVICE).

## 현재 유효한 금지사항

- 순수 로직 결정은 `rosy_control/control/`, `rosy_control/planning/`, `rosy_control/sensing/`, `rosy_control/watch.py`에만 두고 ROS import를 넣지 않는다(AGENTS.md).

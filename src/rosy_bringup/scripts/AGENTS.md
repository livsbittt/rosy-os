...
Do not replace this with `source env.sh` on a real robot (comment in root `env.sh`). Sets `ROS_DOMAIN_ID=40+N` and localhost CycloneDDS — **do not source this for `gz_multi`**.

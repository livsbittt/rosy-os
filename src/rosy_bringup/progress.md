...
gates:
  ...
  LOCAL:
    ...
    cmd: "python3 -m pytest test/test_motor_control.py test/test_bringup_motor_contracts.py test/test_dynamixel_driver_safety.py src/rosy_bringup/test/test_command_deadman.py src/rosy_bringup/test/test_pinky_pro_adapter.py -q"
  ROS-SIM:
    state: HOLD
...
- Dynamixel differential-drive, odometry/TF, 선택적 LiDAR/배터리 publisher, `rosy_core`와 독립된 driver-side `cmd_vel` deadman을 제공한다(D-22).

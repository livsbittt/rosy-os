...
Default robot config. Installed to `share/core/config`. Local override is `~/.rosy/rosy.yaml` (not in this folder). The robot profile and capabilities now live in `src/robots/<model>/config/` (D-196); `robot.model` (default `pinky_pro`, env `ROSY_ROBOT`) picks the package.

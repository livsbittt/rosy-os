...
import math
import unittest
from control.control.recover import is_stuck_motion


class FinalSpeedStuckTest(unittest.TestCase):
    def check(self, elapsed, moved, raw_speed=.005, age=.05):
        path = Path(__file__).parents[1] / 'control/wander/motion.py'
        ...
        node = SimpleNamespace(seen_forward=True, motion_t=Stamp(), have_odom=True,
            ...
            _fwd_speed=lambda:.014, _session_now=lambda:10., session_raw_received=10.-age,
            session_raw_v=raw_speed,
    ...
    def test_stale_raw_output_cannot_suppress_recovery(self):
        self.assertTrue(self.check(1.2, 0., raw_speed=0., age=1.))

    def test_fresh_gate_stop_is_not_motor_stall(self):
        self.assertFalse(self.check(2., 0., raw_speed=0.))

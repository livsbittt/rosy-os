...
- Python LED 서비스 서버(`led_service_server`)로 `rosylib.LED`를 감싸며 `set_led`/`set_brightness` 서비스(`rosy_interfaces`)를 제공한다.
- 모듈 경로는 clean이지만 SOURCE 증거 시험이 미커밋 WIP가 있는 `deploy/robot/Dockerfile`을 읽으므로 `last_verified`는 `uncommitted`다.
- `test/`는 ament 3종 linter뿐이다. 2026-09-15 재현: `python -m pytest src/rosy_led/test/ -q` → 3 errors, `ModuleNotFoundError: No module named 'ament_copyright'`(flake8/pep257도 동일).
- `deploy/robot/Dockerfile`에는 core/io 두 이미지만 있고 rosy_led를 포함하지 않는다.

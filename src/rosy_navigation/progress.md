...
- Nav2와 SLAM Toolbox의 launch/config/맵. Flask UI는 D-3로 `rosy_core` FastAPI에 완전히 대체되어 삭제됐다.
...
- TF/param 정책은 `launch/`가 아니라 `rosy_navigation.*`에 둔다.

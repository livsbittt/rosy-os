...
gates:
  ...
  FIELD:
    state: PARKED
adrs: [D-90, D-94, D-95, D-96, D-97, D-98, D-99, D-100, D-101]
plans:
...
- LOCAL 호스트: 플러그인 심판 + HoldObserver 또는 `--observer overhead`. `--preview`는 노트북 게임 보드 (D-101). CORE `/dashboard` 아님. `host/overhead.py`만 OpenCV.

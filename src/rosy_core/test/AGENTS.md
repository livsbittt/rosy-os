...
| `test_swarm.py` | SWM follow state machine via `from rosy_core.swarm`, 2 Hz cap, stream-loss HOLD |
...
- Swarm tests import `from rosy_core.swarm` (`SwarmManager`, `follow_goal`, `ReferencePose`). Do not import `rosy_core.navigation.swarm`.

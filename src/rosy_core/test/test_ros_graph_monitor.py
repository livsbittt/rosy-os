...
from rosy_core.system.ros_graph import RosGraphMonitor, parse_domain_id
...
def test_snapshot_reports_duplicate_and_foreign_nodes_with_topic_edges():
    node = FakeGraphNode(
        nodes=[
            ('rosy_core', '/rosy_01'),
            ('rosy_bringup', '/rosy_01'),
            ('rosy_bringup', '/rosy_01'),
            ('camera', '/visitor'),
        ],
        topics=[('/cmd_vel', ['geometry_msgs/msg/Twist'])],
        publishers={'/cmd_vel': [endpoint('rosy_core', '/rosy_01')]},
        subscriptions={'/cmd_vel': [endpoint('rosy_bringup', '/rosy_01')]},
    )
    ...
    assert snapshot['edges'] == [
        {'source': '/rosy_01/rosy_core', 'target': '/cmd_vel', 'kind': 'publishes'},
        {'source': '/cmd_vel', 'target': '/rosy_01/rosy_bringup', 'kind': 'subscribes'},
    ]

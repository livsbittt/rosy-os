...
#: 사용 가능 구간의 하단을 걸어 내려갈 때 허용하는 격자 간격. 설계의 스윕 격자
#: (`dock_sweep.py --distances`) 에서 가장 넓은 칸이 0.10 m 다. 이보다 성긴
#: 격자는 자기 간격보다 정확하게 하단을 짚을 수 없고, 이 숫자가 그대로
#: `DockType.docking_threshold_m` 로 들어가므로 성긴 격자로 추측하지 않는다.

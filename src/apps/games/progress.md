...
gates:
  SOURCE:
    ...
    cmd: "python -m pytest src/games/test/test_games_boundaries.py test/test_games_surface.py -q"
  LOCAL:
    state: GO
    evidence: "D-112 stair 1 visibility report; D-113 host track closed (2026-09-18 Windows)"
    cmd: "python -m pytest src/games/test test/test_games_surface.py -q"
  ROS-SIM:
    state: N/A
  ARTIFACT:
    state: N/A
...
1. D-96 계단 1 실측: `--stair 1 --observer overhead --preview`

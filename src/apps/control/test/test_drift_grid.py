"""Odometry drift grid: prototype A against prototype B, same injected error.
...
while moving, from the believed start). Both followers are built at the
believed start and run under CORE's 3 s lease. Slow (240 closed-loop runs,
...
def grid(request):
    jobs = [(kind, cell, index) for kind in "AB" for cell in CELLS for index in range(12)]
    ...
    table = {(kind, cell): [r for r in runs if r["kind"] == kind and r["cell"] == cell]
             for kind in "AB" for cell in CELLS}

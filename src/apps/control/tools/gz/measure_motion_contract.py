...
def _source_root() -> Path:
    ...
    if not root.is_dir():
        label = "ROSY_SOURCE_ROOT" if configured else "absorbed control package"
        raise RuntimeError(f"{label} does not exist: {root}")
    if not (root / "package.xml").is_file() or not (root / "control").is_dir():
        raise RuntimeError(f"ROSY_SOURCE_ROOT must contain package.xml and control/: {root}")
    return root
...
def main():
    ...
    runtime_root = source_root / 'control'

...
- `deploy/robot/Dockerfile`에는 core/io 두 이미지만 있고 rosy_sensor_adc를 포함하지 않는다. wiringPi I2C는 Pi 전용 의존성이다.
...
- `power/mode` 정책 판단은 `rosy_core`가 소유한다. 이 노드는 구독만 하고 정책을 다시 구현하지 않는다.

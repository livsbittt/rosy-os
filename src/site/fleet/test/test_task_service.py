...
def test_manual_navigation_uses_shared_validation_and_persists_receipt(tmp_path):
    ...
    task = run(service.submit_navigation(
        ...
        source="operator", actor_id="site-console", request_key="manual-1", dispatch=dispatch,
    ))
    ...
    assert task["receipt"] == {"accepted": True, "queued": False}
...
def test_definite_command_rejection_records_failed_without_storing_raw_error(tmp_path):
    ...
    task = run(service.submit_navigation(
        ...
        actor_id="site-console", request_key="rejected-1", dispatch=dispatch,
    ))
...
def test_local_hub_rejection_is_failed_without_claiming_unknown_dispatch(tmp_path):
    ...
    task = run(service.submit_navigation(
        robot_id="rosy_01", x=1.0, y=2.0, yaw=0, source="operator",
        actor_id="site-console", request_key="blocked-1", dispatch=dispatch,
    ))

...
1. Get the code onto the Pi. Either
   ...
   b. installed: ``scp src/runtime/control/tools/device/hotpath_measure.py pinky@<pi>:/tmp/``
...
2. Bench, with the robot stack stopped so nothing competes for the CPU (about 1-2 min):
       cd "<checkout>/src/runtime/control"
...
#: src/runtime/control when run from a checkout; a lone copied file has no package root.

...
from control.planning import FREE, OccupancyMap, GoalBrain
from control.planning.frontier import pick_goal
...
def test_nearest_strategy_changes_target_without_changing_clearance():
    ...
    with patch('control.planning.frontier.frontier_points',return_value=clusters()):
...
def test_nearest_preserves_preferred_clearance_pass_over_shorter_retry():
    m=OccupancyMap(25,25,.1,fill=FREE)
    with patch('control.planning.frontier.frontier_points',return_value=clusters()), \
            patch('control.planning.frontier._reachable_costs',side_effect=[{(15,10):1.4},{(3,10):.2,(15,10):1.4}]), \
            patch('control.planning.frontier.nearest_free',side_effect=lambda inflated,cell,**kwargs:cell):
...
def test_invalid_strategy_rejected_and_brain_default_stays_gain():
    ...
    with patch('control.planning.goals.pick_goal',return_value=None) as pick:

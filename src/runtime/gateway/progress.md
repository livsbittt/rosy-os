...
gates:
  SOURCE:
    ...
    cmd: "python3 -m pytest src/runtime/gateway/test/test_module_criteria.py test/test_control_absorption_package.py -q"
  LOCAL:
    state: GO
    ...
    cmd: "PYTHONPATH=src/runtime/gateway:src python3 -m pytest src/runtime/gateway/test -q"
  ROS-SIM:
    state: GO

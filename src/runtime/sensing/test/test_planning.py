...
from control.planning import (FREE, OCC, UNKNOWN, GoalBrain,
...
class GoalBrainTest(RoomCase):
    ...
    def test_probe_rests_after_dead_target(self):
        ...
        m = self.known_room()
        brain.covered = set(m.free_cells())
        with mock.patch('control.planning.goals.best_route',
                        return_value=None):
...
class ArrivalProgressionRegression(unittest.TestCase):
    ...
    def test_completed_frontier_neighborhood_excluded_on_immediate_replan(self):
        ...
        with mock.patch('control.planning.goals.pick_goal', return_value=None) as pick:
            brain.plan(m, target)

...
- **보안 리뷰 반영(PR #35):** "로그인 유지"는 만료가 7일 이내인 페어링 토큰만 `localStorage`에 두고, CORE는 설정한
  페어링 수명도 168 h로 자른다. 로그아웃은 토큰이 있으면 항상 `POST auth/logout`을 부르고 409면 이 브라우저에서만 잊는다.
  폐기 문구는 두 경우(로봇 화면 코드 → 재부팅·`sudo rosy-login-code`, 관리자 등록 코드 → 관리자에게 새 코드)를 다 말한다.
  WebSocket 백오프는 소켓이 10 s 살아 있어야 되돌린다. 회전 스크립트는 새 토큰에 `expires_at`이 있으면(생성 응답이든
  whoami든) 되돌리고, 저장소 교체·다시 읽기가 실패하면 `.previous`를 되돌린 뒤 새 id를 지운다. `http://`이면 경고한다.
  CORE에는 인증 없이 장치를 증명하는 경로(`device_uid`)가 없어 Bearer를 보내기 전 대조는 못 한다 — 위협과 운영 조건을
  스크립트 설명과 런북에 적었다.
- **하지 않은 것:** 대시보드의 등록 코드 발급 UI(`POST auth/enrollment-codes`)는 S3 목록에 없어 넣지 않았다.

**부분 구현 기록 (2026-09-25, Proposed 유지):** S1/S3가 착지했다(`api/v1/auth.py` 존재, API Ref v1.19 문서화).
단 S4 실기 평가표(6a–6g)와 등록 코드 발급 UI가 미완이므로 Accepted로 뒤집지 않는다.

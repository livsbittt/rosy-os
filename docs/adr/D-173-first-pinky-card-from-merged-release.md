## D-173 첫 Pinky Pro 카드는 머지된 커밋의 서명 이미지를 검토된 plan에 고정해 굽는다
...
| 7 | 카드 부팅, first-boot `PROVISIONED`, readback JSON (runbook G0-G2) | 부분 — PROVISIONED·Wi-Fi 정상, 복구 gate import 실패로 CORE 미기동. [D-174](D-174-first-boot-defects-and-boot-indicator.md)로 이관 |

...
3. **추출 규칙 (트랙 1·2 공통).**
   ...
   - **(d) 개정 (2026-09-23, 사용자 승인):** 이 규칙을 처음 적용하자 두 가지가 드러났다. rig를
     띄우는 `run_track260905.sh`는 흡수 때 빠져 있었다(이식해 복구했고, 참조 가드
     `test_rig_script_references.py`를 붙였다). 또 한 프로세스 모드(`RIG_COMPONENT=all`)는 기준본(HEAD)에서도
     시험 주행 도중 게이트 출력 신선도(0.25 s)를 잃고 실패한다. 그래서 (d)의 통과 조건을 다음 셋으로
     고친다. (1) 분리 모드 기본 시나리오 `ready`, (2) 비상정지 음성 사례 `failed`, (3) 한 프로세스
     모드는 기준본과 결과가 같을 것(A/B 동등). rig는 WSL의 ext4 복사본에서 돌린다. `/mnt/f`에서 돌리면
     오도메트리 나이가 허용치(0.2 s)를 넘은 사례가 있다. 한 프로세스 모드 결함은 평가 문서 §8의
     미해결 항목이다.

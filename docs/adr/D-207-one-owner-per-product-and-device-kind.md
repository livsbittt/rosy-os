...
4. **감지의 자리는 `src/core/control` 이다.** 카메라, 차선, 보정, `road/observation` 은 여기 남는다. 최종 `cmd_vel` 은 `core` 다. `sensing` 프로파일이 속도를 내지 않는 규칙은 [D-208](D-208-sensing-profile-publishes-no-velocity.md)이다. 데스크 미로 맵은 control 묶음에 남는다. Nav2 맵은 `navigation` 에 남는다. Gazebo 월드 중 control 묶음이 아닌 것은 `sim/gz_sim` 에 남는다. 맵 폴더를 새로 만들지 않는다. `control` 의 계획·배회·레거시 안전 노드를 패키지로 떼는 일은 D-205 P1–P3 안에서만 한다.

...
- **장치가 없을 때.** LCD·SPI·GPIO 칩이 없으면 5 s 간격 6회(udev가 그룹을 붙이는 시간) 시도하고, 첫 실패와 포기를
  한 번씩만 기록한 뒤 LCD 없이 돈다. LCD도 부저도 없으면 0으로 끝나 재시작 루프가 없다. 배터리 버스가 없으면 `--`로
  그리고 조용히 다시 시도한다.
- **gpiochip4.** Pi 5의 40핀 헤더 GPIO는 RP1(label `pinctrl-rp1`)이다. Ubuntu 24.04 raspi 커널에서는 `gpiochip4`로 알려져
  ...
  않고 한 번 기록한다. unit은 `RPI_LGPIO_CHIP=4`를 준다(그 변수를 따르는 rpi-lgpio 버전에서 칩을 고정한다). 번호가
  다르면 `DeviceAllow`·udev 규칙·`board.yaml`·시험을 한 변경에서 바꾼다.
...
- **부저.** 기본 BCM 22, **기본 꺼짐.** `ROSY_BUZZER_ENABLED`는 `true`/`false`만 받고(그 밖은 꺼짐),
  `ROSY_BUZZER_PIN`은 LCD 선(18·25·27)이 아닌 헤더 BCM만 받는다. 켜는 방법은 사람이 장치에서 소리를 확인한 뒤
...
- **이미지.** customizer가 apt `python3-spidev`·`python3-rpi-lgpio`·`python3-numpy`·`python3-pil`·`fonts-dejavu-core`를
  ...
  import, 네 단계 카드 렌더, 폰트, unit 샌드박스·enable을 확인한다. Pi가 아닌 빌더에서 `RPi.GPIO` import 거부는
  `/proc/device-tree/model`에 Pi가 없을 때만 허용한다. 마운트 검사기는 unit enable, udev 규칙, dpkg 설치 상태,
  `dtparam=spi=on`을 본다.

...
1. 천장 스마트폰 앱은 JPEG 프레임을 `rosy-overhead/1` WebSocket `/overhead/v1/frames`로 보낸다. 수신기는 source별 Bearer token을 handshake와 hello의 `source`를 대조해 검증하고, source별 최신 프레임 1장과 통계만 관리한다. CLI는 지정 source용 token 하나로 시작한다. 마커 인식과 Fleet 전송은 없다.

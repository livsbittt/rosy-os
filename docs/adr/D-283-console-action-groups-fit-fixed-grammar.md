...
4. **전환은 안전 정지와 함께 처리한다.** 수동 운전 중 다른 그룹을 선택하면 기존 teleop을 먼저 중지하고 0 명령을 전달한 뒤 패널을 숨긴다. 탭 전환은 teleop 세션을 유지하거나 명령을 재개하지 않는다. 라우트 이동과 패널 해제도 기존 unmount 정지 동작을 유지한다.
...
| 단계 | 확인할 것 | 통과 기준 |
|---|---|---|
| G1 | 그룹 선택, panel mount/unmount 및 정지 동작 시험 | 그룹 전환 때 활성 teleop의 terminal zero가 전달되고 그룹 밖에서 명령이 재개되지 않음. Viewer에는 조작 그룹 없음. |

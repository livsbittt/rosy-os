...
현장 구성: 로봇이 상위 WiFi(공유기)에 STA로 연결된 상태에서 **동시에 자체 AP를 제공해 무선을 릴레이**한다. 사용자 기기는 로봇 AP에 접속하여 로봇에 직접 접근한다.

### NET-001 WiFi 릴레이 모드 (AP+STA 동시)

로봇은 상위 WiFi 연결(STA)을 유지하면서 동시에 자체 AP를 제공할 수 있어야 한다. NetworkManager AP 모드 + shared 기반으로 프로비저닝한다. 라디오 제약(AP는 상위와 동일 채널)을 문서에 명시한다.

### NET-002 단일 진입점

릴레이 AP 서브넷에서도 표준 진입점(`http://rosy-01.local:8080`)으로 접근 가능해야 한다. mDNS 미지원 클라이언트를 위한 폴백으로 AP 게이트웨이 IP(예: `http://10.42.0.1:8080`)를 접속 가이드에 문서화한다.

### NET-003 업링크 단절 시 로컬 유지

상위 WiFi 단절 시에도 로봇 AP 서브넷 내에서의 접속·정보 조회·제어(Teleop·Navigation·E-Stop)가 유지되어야 한다. Local-First 원칙(§1.3)의 네트워크 표현이다.

### NET-004 Fleet 업링크 경유

Fleet 접속(로봇 아웃바운드 WS)은 상위 WiFi 업링크를 경유한다. 릴레이 NAT 하에서도 로봇이 접속을 개시하므로(ADR-D-5) 인바운드 포트 개방이 필요 없어야 한다.

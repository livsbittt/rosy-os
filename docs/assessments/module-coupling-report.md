...
- `runtime/sensing → core_*`: **0건**. `control` 내부 import만 존재. 선언과 일치하며, 흡수 원칙(CORE가 최종 명령 소유, control은 증거 생산) 유지.
...
- `control`은 `cmd_vel_raw`, `wander/cmd`, `calib/*`, `camera/*` 발행, `scan/odom/imu_raw/us_sensor` 구독. 단, 레거시 잔재 2곳이 최종 토픽을 **구독**함:
  - `src/runtime/sensing/control/web_node.py:431` — `create_subscription(Twist, 'cmd_vel', ...)`
  - `src/runtime/sensing/control/wander/node.py:36` — 동일 패턴

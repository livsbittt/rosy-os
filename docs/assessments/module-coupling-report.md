...
1. `src/core/core/package.xml`에 `control` `exec_depend` 추가 **또는** 어댑터를 `core_features`/`core_api_web`과 같은 명시 경계로 이동. 현 상태는 빌드·패키징이 코드 실체를 거짓말함. (`core/core/core/bridge/control_sensor_adapter.py`)

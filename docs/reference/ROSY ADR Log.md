...
**추종은 언제 끝나는가.** follow() 의 게이트는 시작 시점만 본다. 대형은 몇 분씩
유지되므로 매 틱 다시 본다: e-stop 이 걸리면 추종을 **취소**한다(중단이 아니라
해제 — 해제 뒤 참조 프레임 하나로 다시 달리기 시작하면 안 되고, 운영자가 다시
명령해야 한다). 도킹이 진행 중이어도 취소한다: DOCKING(4) > NAVIGATION(5) 이고
둘이 같은 Nav2 액션을 쓰므로, 양보하지 않으면 추종이 SAF-005 저배터리 복귀
주행을 계속 선점한다.

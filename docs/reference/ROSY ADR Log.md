...
**추종은 언제 끝나는가.** 세 가지가 세션을 끝낸다: 운영자의 cancel, e-stop,
그리고 NAV-006 stuck. e-stop 은 중단이 아니라 **해제**다 — 해제 뒤 참조 프레임
...
**세션 토큰.** 목표에는 `NavigationManager` 가 발급한 세션 토큰이 붙는다.
...
이어지고, HOLD 중에도 목표의 임자는 여전히 추종 세션이다.

...
>
> All of this lives on branch `feat/dock-detector-measurement-rig`, which is **unmerged** as
> of writing. The repo has no configured remote, so there are no PR numbers; commit SHAs are
> bare branch commits that any rebase or squash will rewrite. Treat them as pointers into
> that branch's history, not as identifiers.
>

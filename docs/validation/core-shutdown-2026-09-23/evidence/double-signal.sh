...
WS=/root/rosy_shutdown_after
N=$WS/src/core/core/core/node.py
cp $N /root/rosy_shutdown_diag/node.py.orig
python3 - $N <<'PY'
...
OUT=$D/out/double-after/summary.txt; mkdir -p $D/out/double-after
...
for SIG in INT TERM; do for n in 1 2 3; do
  H=/root/rosy_shutdown_home/d$SIG$n; rm -rf $H; mkdir -p $H/.rosy; printf "network:
  api_port: 18480
  api_host: 127.0.0.1
" > $H/.rosy/rosy.yaml
  ...
  cp $H/console.log $D/out/double-after/SIG$SIG-run$n-console.log
done; done
cp /root/rosy_shutdown_diag/node.py.orig $N

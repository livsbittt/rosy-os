...
- **남은 이관:** `/console`의 기존 주행·지도·카메라 조작, `/setup`의 초기 위치/SLAM/도크 준비 작업, `/device`의 네트워크·ROS·설정·하드웨어 관리 기능은 단계별로 옮긴다. 각 단계에서 레거시 `/dashboard`를 유지하고 동등 동작을 확인한 뒤 이관 범위를 넓힌다.
- **Task 4 남음:** 호스트·네트워크·ROS 그래프·릴리스·신원·토큰·안전 한계 패널과 기존 `점검` 뷰 카드 정리는 별도 단위로 남아 있다.

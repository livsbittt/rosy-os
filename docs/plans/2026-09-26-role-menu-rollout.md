...
- **남은 이관:** `/device`의 ROS 그래프 및 Host Agent 네트워크/릴리스 조작은 아직 새 패널에서 수행하지 않는다. `/setup`의 도크 종류·도크 등록 관리와 기존 설정 카드 정리, `/console`의 운전·지도·카메라·모드·도킹 실행 이전도 남아 있다. 텔레옵 hold/포커스/연결 해제 zero 전송 동등성을 확보하기 전까지 기존 운전 화면을 제거하지 않는다.

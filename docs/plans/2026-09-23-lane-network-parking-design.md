...
| `acquire_creep_m` | 0 | 0.20 | 태그가 안 보일 때 오도메트리 직진 허용 거리 |
| `backoff_m` | – (시간 기준 `backoff_s`) | 0.10 | 재착좌·재시도 후진을 거리로 끝낸다(스퍼 뒤는 벽) |
...
2. **ACQUIRING**: 태그가 보일 때까지 방위를 유지하며 크리프(0.03 m/s)로 최대 0.20 m
   전진. 진입점에서는 태그 윗단이 시야 밖이라(§1 식) 주차점 약 0.1 m 앞(진입점에서 약 0.17 m)까지
   들어가야 보인다.

...
- SIM 환경 실태(2026-09-21 조사): WSL Ubuntu에 Gazebo Sim 8.15.0 있음. 단 `ros_gz` 미설치 — `apt install ros-jazzy-ros-gz` 필요. 병렬 세션이 `~/rosy_ws`(SIM 스택: fleet·gz_sim·navigation)를 활성 빌드 중이므로 실측은 그 워크스페이스 위에서, 빌드 안정 후 수행할 것. 참고: 오버레이 `install/setup.bash` 소싱이 PYTHONPATH를 덮어쓰는 사례 확인 — 언더레이(/opt/ros) 소싱 후 파이썬 경로는 append로 재구성할 것

...
| `node.py` — `executor` → `_executor` | **Accepted, private by necessity.** rclpy 7.1.x (Jazzy) `MultiThreadedExecutor` never shuts down its `ThreadPoolExecutor`, and `Executor.shutdown()` does not wait for in-flight callbacks, so `_stop_executor` drains that pool itself at teardown (core-shutdown 2026-09-23). The reach is `isinstance`-guarded: a future rclpy that renames or drains the pool makes it a no-op, not a failure. Remove it when core moves to an rclpy whose executor shutdown drains the pool. |

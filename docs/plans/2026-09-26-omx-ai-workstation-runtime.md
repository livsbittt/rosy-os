...
3. **Implemented locally:** the opt-in software-only profile runs the locked ROBOTIS OMX-F Gazebo launch. A small checked patch adds Gazebo server-only mode, so it needs no host display. It has no serial/camera grants, uses an isolated ROS domain, and starts the vendor URDF with Gazebo's `gz_ros2_control` simulation plugin. This validates the vendor simulation path; it does not validate physical Dynamixel bringup or Rosy's bounded arm-command adapter. Gazebo expands the workstation image by roughly 1.7 GB of installed dependencies in the current build; keep simulation optional.

...
def _reports(directory: Path) -> list[Path]:
    return sorted(directory.glob("boot-*.json"))
...
def needs_write(boot_partition: Path, record: dict) -> bool:
    ...
    if existing is None:
        return True
    try:
        return json.loads(existing.read_text(encoding="utf-8")).get("stage") != record["stage"]
...
def write(boot_partition: Path, record: dict, tails: dict[str, list[str]]) -> Path:
    ...
    if target is None:
        numbers = [int(path.stem.split("-")[1]) for path in _reports(directory)
                   if path.stem.split("-")[1].isdigit()]
        target = directory / f"boot-{(max(numbers) + 1) if numbers else 1:04d}-{boot_id[:8]}.json"

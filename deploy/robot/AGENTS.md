...
| `Dockerfile` | Development/CI-only legacy container build. The `core` stage copies `interfaces`, `core` and `src/products/pinky_pro` (D-196; `.dockerignore` allow-lists `src/products/pinky_pro/**`). Pre-existing gap: it does not copy `core_common`/`core_events`/`core_features`/`core_api_web`, so that stage is not a working CORE build today |
